...
INACTIVITY_TIMEOUT_SECONDS = 50  # 5 minutes
...
with st.sidebar:
    ...
    rules_status = "Loaded" if st.session_state.get('config_loaded') else "Not loaded"
    rules_class = "status-loaded" if st.session_state.get('config_loaded') else "status-missing"
    st.markdown(f"**Rules** <span class='status-badge {rules_class}'>{rules_status}</span>", unsafe_allow_html=True)
    
    template_status = "Loaded" if st.session_state.get('template_path') else "Not loaded"
    template_class = "status-loaded" if st.session_state.get('template_path') else "status-missing"
    st.markdown(f"**Template** <span class='status-badge {template_class}'>{template_status}</span>", unsafe_allow_html=True)
    
    wishes_status = "Loaded" if st.session_state.get('wishes_path') else "Not set"
    wishes_class = "status-loaded" if st.session_state.get('wishes_path') else "status-missing"
...
if not st.session_state.get('config_loaded', False):
    # If Rules_edit.xlsx exists but session says not loaded, try to load it
    if os.path.exists(RULES_FILE):
        try:
            config = load_config(RULES_FILE)
            st.session_state['config'] = config
...
if not st.session_state.get('config_loaded', False):
    ...
    Once you upload a valid Rules file (in sidebar), this page will be replaced with the full featured interface.
    """)
    st.info("""
